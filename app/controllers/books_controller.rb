class BooksController < ApplicationController
  ...
  def index
    ...
    @books = @q.result(distinct: true).includes(:author, :best_lines, :user,
                                                :tastes).page(params[:page]).per(10)
  end
  ...
  def book_params
    params.require(:book).permit(:name, :publishing_year, :author_id, :cover,
                                 :user_id)
  end
end
